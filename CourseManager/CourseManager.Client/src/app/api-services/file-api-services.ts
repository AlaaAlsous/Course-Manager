...
export class FileApiService {
    ...
    baseUrl = 'https://localhost:5053/api/files';
    ...
    async getCourseSectionFiles(courseSectionId: number): Promise<FileAsset[]> {
        try {
            const response = await fetch
            (`${this.baseUrl}/course-section/${courseSectionId}`);
    ...
    async getFileContent(fileAssetId: number): Promise<string | null> {
        try {
            const response = await fetch
            (`${this.baseUrl}/${fileAssetId}/content`);
    ...
    if (!response.ok) {
      throw new Error(
        `Error updating file content: ${response.status}`
      );
    }
...
async downloadFile(fileAssetId: number): Promise<Blob | null> {
  try {
    const response = await fetch(
      `${this.baseUrl}/${fileAssetId}/download`
    );

    if (!response.ok) {
      throw new Error(
        `Error downloading file: ${response.status}`
      );
    }
...
async uploadFile(
  ...
  try {
    ...
    const response = await fetch(
      `${this.baseUrl}/upload/${entityType}/${entityId}`,
      {
        method: 'POST',
        body: formData
      }
...
async removeFileFromCourse(
    ...
    try {
        const response = await fetch(
            `${this.baseUrl}/course/${courseId}/${fileAssetId}`,
            {
                method: 'DELETE',
            }
        );
        ...
        if (!response.ok) {
            throw new Error(
                `Error removing file from course: ${response.status}`
            );
        }
...
async removeFileFromCourseSection(
    sectionId: number,
    fileAssetId: number
): Promise<void> {
    try {
        const response = await fetch(
            `${this.baseUrl}/course-section/${sectionId}/${fileAssetId}`,
            {
                method: 'DELETE',
            }
        );
        ...
        if (!response.ok) {
            throw new Error(
                `Error removing file from course section: ${response.status}`
            );
        }
        ...
        console.error(
            'Error removing file from course section:',
            error
        );
    }
}
async removeFileFromGroup(
  ...
  try {
    const response = await fetch(
      `${this.baseUrl}/group/${groupId}/${fileAssetId}`,
      {
        method: 'DELETE'
      }
    );
    ...
    if (!response.ok) {
      throw new Error(
        `Error removing file from group: ${response.status}`
      );
    }
...
async removeFileFromPerson(
  personId: number,
  fileAssetId: number
): Promise<void> {
  try {
    const response = await fetch(
      `${this.baseUrl}/person/${personId}/${fileAssetId}`,
      {
        method: 'DELETE'
      }
    );
    ...
    if (!response.ok) {
      throw new Error(
        `Error removing file from person: ${response.status}`
      );
    }
